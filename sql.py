...
with open(filename,'r') as log:
	for line in log:
		...
		if('OPEN_START' in line):
			line = line.strip()
			columns = line.split()
			open_start = columns[3][:-1]
			print(open_start)
		elif('OPEN_END' in line):
			line = line.strip()
			columns = line.split()
			open_end = columns[3][:-1]
...
with open(filename,'r') as log:
	print(open_start,log.readline())
	while(not(open_start in log.readline())):
		pass
	line = log.readline()
	while(not(open_end in line)):
...
sched_timestamps = np.array(sched_timestamps) - float(open_start)*1000
...
block_insert_timestamps = np.array(block_insert_timestamps) - float(open_start)*1000
block_complete_timestamps = np.array(block_complete_timestamps) - float(open_start)*1000
...
plt.plot([start,start],[5,25])
plt.plot([end,end],[5,25])
for i in range(4):
	line, = plt.plot(cpu_frequency_timestamps[i],cpu_frequency[i])
	if(i == cpuid):
